...
export default function OrdersPage() {
    ...
    useEffect(() => {
        fetch("/api/orders")
            .then((r) => r.json())
            .then((body) => {
                if (body.ok) setOrders(body.data);
                else setError(body.error?.message ?? "Failed to load orders");
            })
        ...
        <main className="flex-1 flex flex-col px-6 lg:px-30 py-10 w-full overflow-x-hidden">
            <div className="flex flex-col gap-[10px] items-start w-full">
                ...
                <div className="relative flex flex-row items-center justify-center gap-4 w-full pb-6 lg:pb-4 z-10 bg-white">
                    ...
                    <Squircle borderRadius={20} smoothing={50} className="bg-[#0000f4] w-[300px] lg:w-[500px] h-[60px] lg:h-[60px] shrink-0">
                ...
                <div className="flex flex-col gap-12 lg:gap-8 w-full max-w-[500px] lg:max-w-[600px] self-center items-center">
                    {loading ? (
                        <div className="mt-[30px]">
                            <SvgText text="Loading…" weight="500" height={16} className="text-[#aaaaaa]" />
                        </div>
                    ) : error ? (
                        <div className="mt-[30px]">
                            <SvgText text={error} weight="500" height={16} className="text-[#e53e3e]" />
                        </div>
                    ) : orders.length === 0 ? (
                        <div className="mt-[30px]">
