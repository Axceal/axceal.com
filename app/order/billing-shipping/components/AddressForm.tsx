 ...
 * `form` (AddressFormState) bundles state and refs together, so the
 * react-hooks/refs rule false-flags every `form.X` access as a ref read
 * during render — even for plain string fields like `form.first` or
 * `form.fieldErrors.zip`. The actual `.current` property is never touched
 * here (refs are only forwarded as `ref={form.xRef}`, which is allowed).
 * Splitting refs into a sub-object would silence the rule but is a wider
 * refactor — not security-relevant. Tracked in §F1.6. */
...
function FieldError({ msg }: { msg?: string }) {
    if (!msg) return null;
    return <SvgText text={msg} weight="500" height={12} className="text-[#ff0000] self-center mt-1" />;
}
...
const corrPill = "bg-[#ff0000] text-[#1e1e1e] rounded-full px-5 py-[18px] ring-1 ring-[#0000f4]/40";
        ...
        <div className="relative flex flex-col gap-4 w-full max-w-[430px]">
            ...
            <div className="w-full flex flex-col gap-2">
                <div ref={form.addressRef} className="w-full">
                    <SvgInput
                        value={form.address}
                        onChange={(v) => { if (v.length <= 50) { form.setAddress(v); form.clearFieldError("address"); } }}
            ...
            <div className="w-full flex flex-col gap-2">
                ...
                {(form.zipError || form.fieldErrors.zip) && (
                    <div className="w-full flex justify-center">
                        <SvgText text={form.zipError ?? form.fieldErrors.zip ?? ""} weight="600" height={12} className="text-[#ff0000] mt-1" />
                    </div>
                )}
            </div>
            ...
            <div className="w-full flex flex-col gap-2">
                ...
                >
                    ...
                    <SvgInput
                        ...
                        value={form.phone2}
                        onChange={v => { form.setPhone2(v.replace(/\D/g, "").slice(0, 5)); form.clearFieldError("phone"); }}
                        onKeyDown={e => {
